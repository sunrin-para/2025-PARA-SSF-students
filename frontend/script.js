document.addEventListener("DOMContentLoaded", () => {
    ...
    async function initializeApp() {
        try {
            ...
            addMessageToUI("system", "초기 데이터를 불러오는 데 실패했어요.", "error");
        } finally {
        ...
        try {
            ...
            for (const func of functionCalls) {
                if (func.name === "generate_playlist") {
                    ...
                    addMessageToUI("system", `알 수 없는 요청이에요: ${func.name}`, "error");
                }
            }
            ...
            addMessageToUI("system", `죄송해요, 오류가 발생했어요: ${error.message}`, "error");
        } finally {
        ...
        try {
            ...
            if (playlistData.playlist) {
                ...
                addMessageToUI(
                    "system",
                    `플레이리스트를 만드는 데 실패했어요: ${
                        playlistData.message || "알 수 없는 오류"
                    }`,
                    "error",
                );
            }
            ...
            addMessageToUI(
                "system",
                `플레이리스트 생성 중 오류가 발생했어요: ${error.message}`,
                "error",
            );
        }
    }
    ...
    async function handleGeneralMessage(messageText) {
        try {
            ...
            addMessageToUI("system", `메시지 전송 중 오류가 발생했어요: ${error.message}`, "error");
        }
    }
    ...
    async function handleUpdatePreferences(args) {
        ...
        try {
            ...
            addMessageToUI(
                "system",
                `선호도 업데이트 중 오류가 발생했어요: ${error.message}`,
                "error",
            );
        }
    }
        ...
        try {
            ...
            if (Array.isArray(data.data)) {
                data.data.forEach(async (msg) => {
                    if (msg.role === "system" && msg.content === "playlist_ui") {
                        await renderPlaylistToUI(msg.data);
                    } else {
                        if (msg.content === "generate_playlist") {
                            msg.content = "플레이리스트를 생성했어요!";
                        } else if (msg.content === "update_preferences") {
                            msg.content = "선호도를 업데이트했어요!";
                        }
    ...
    function updateSidebarDisplay(playlistData) {
        ...
        sidebarCurrentPlaylistTitle.textContent = "당신을 위한 플레이리스트";
